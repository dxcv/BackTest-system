...
#     industry_data = pd.read_sql_query('select wind_code,ind_code, from industry', engine).dropna()
...
def linear_params_avg(factor_value,factor_name,return_value,industry,date,H,N,return_period,method):
    ...
    for i in range(N):
        if method=='OLS':
            ...
            params = linear_regression_WLS(factor_value, factor_name, return_value, datelist[i], return_period, industry)
...
def buy_stock_select(date,buy_available,cash_account,factor_value,factor_name,return_value,industry,H,N,return_period,method,num,open_price,regression_method,weight):
    ...
    if weight=='mkt_size':
        mkt_size=pd.read_csv('mkt_cap_ard.csv')
        mkt_size=mkt_size.ix[date,buy_code]
        mkt_size=mkt_size/mkt_size.sum()
        for stock in buy_code:
            price = open_price.ix[date, stock]
            stock_weight=mkt_size[stock]
            cash_available =0.995* stock_weight * cash_account
...
#在调仓截面进行交易
def back_test(initial_cash,start_date,end_date,return_period,factor_value,factor_name,return_value,H,N,num=20,method='linear',regression_method='OLS',weight_method='equal'):
    ...
    for date in trade_day_list:
        ...
        if(date in date_list):
            ...
            buy_stock_list = buy_stock_select(date, buy_available, cash_account, factor_value, factor_name,
                                              return_value, industry, H, N, return_period, method, num, open_price,regression_method,weight_method)
...
#回测
t0=time.time()
# head=[['H','N','return_period','num','method','ratio']]
# i=1
# for H in [1,2]:
#     for N in [1,2,3,4]:
#         for return_period in [20]:
#             for num in [20,40]:
#                 [total_account1, deal_table1] = back_test(1000000, 20130701, 20170931, return_period,
#                                                           [mkt, pb, momentum, turn], ['mkt', 'pb', 'momentum', 'turn'],
#                                                           return_value, H, N, num, 'linear', 'OLS')
#                 [total_account2, deal_table2] = back_test(1000000, 20130701, 20170931, return_period,
#                                                           [mkt, pb, momentum, turn], ['mkt', 'pb', 'momentum', 'turn'],
#                                                           return_value, H, N, num, 'linear', 'WLS')
#                 [total_account3, deal_table3] = back_test(1000000, 20130701, 20170931, return_period,
#                                                           [mkt, pb, momentum, turn], ['mkt', 'pb', 'momentum', 'turn'],
#                                                           return_value, H, N, num, 'IC_score', 'WLS')
#                 sharp_ratio1 = summary(total_account1)
#                 sharp_ratio2 = summary(total_account2)
#                 sharp_ratio3 = summary(total_account3)
#                 head.append([H,N,return_period,num,'OLS',sharp_ratio1])
#                 head.append([H, N, return_period, num, 'WLS', sharp_ratio2])
#                 head.append([H, N, return_period, num, 'IC', sharp_ratio3])
#                 print(i)
#                 print(time.time()-t0)
#                 i+=1
# with open('sharp_ratio2.csv','w',newline='') as csvfile:
#     writer=csv.writer(csvfile)
#     writer.writerows(head)
[total_account1, deal_table1] = back_test(1000000, 20130701, 20170931, 20,[mkt, pb, momentum, turn], ['mkt', 'pb', 'momentum', 'turn'],return_value, 0, 2, 20, 'linear', 'OLS','mkt_size')

# 计算指标
sharp_ratio=summary(total_account1)
#
# zz500=pd.read_csv('zz500.csv',index_col=0,encoding='gbk').ix[:-2,:]
# zz500.index=map(lambda x:int(datetime.datetime.strftime(datetime.datetime.strptime(x,"%Y/%m/%d"),"%Y%m%d")),zz500.index)
# total_account1['zz500_p']=zz500['收盘价(元)']
# num=1000000/zz500.ix[20130701,'开盘价(元)']
# total_account1['zz500_v']=total_account1['zz500_p']*num
# total_account1['hedge']=total_account1['account_value']-total_account1['zz500_v']
# print(total_account1)
# xs = [datetime.datetime.strptime(d, '%Y%m%d').date() for d in list(total_account1.index.map(str))]
# line1=plt.plot(xs, list(total_account1['account_value']))
# line2=plt.plot(xs, list(total_account1['zz500_v']))
# plt.legend([line1, line2], ["净值曲线", "中证500"], loc=1)
# plt.grid(True)
# plt.xlabel('Date')
# plt.ylabel('account_value')
# plt.show()
#
# xs = [datetime.datetime.strptime(d, '%Y%m%d').date() for d in list(total_account1.index.map(str))]
# plt.plot(xs, list(total_account1['hedge']))
# plt.grid(True)
# plt.xlabel('Date')
# plt.ylabel("ex_return")
# plt.show()